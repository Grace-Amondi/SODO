...
###In the News
For current news regarding Sodo, see our [Wiki Page](https://github.com/ngageoint/sodo/wiki/In-The-News)

### Screenshots
![Sodo Main page](https://cloud.githubusercontent.com/assets/147580/3464387/e58da414-024b-11e4-9a02-f9074f26047e.png)
...
The ``sodo/settings.py`` file contains installation-specific settings. The Database name/pw and server URLs will need to be configured here.
...
1. You can optionally deploy Sodo with all dependencies to a Virtual Machine or a cloud VM (such as an Amazon Web Services EC2 box) by using the chef installer at [https://github.com/ngageoint/sodo-chef-installer](https://github.com/ngageoint/sodo-chef-installer)
...
4. Install and setup sodo:

        % mkdir -p ~/pyenv
        % virtualenv --no-site-packages ~/pyenv/sodo
        % source ~/pyenv/sodo/bin/activate
        % git clone https://github.com/ngageoint/sodo
...
6. Modify local settings (Modify entries below based on your system settings. Hit Ctl + D to save local settings file.):
        ...
        % cat > local_settings.py
          ...
          DEFAULT_FROM_EMAIL = "sodo-alerts@server.com"
          MEDIA_ROOT = '/opt/src/pyenv/sodo/sodo-django'
          STATIC_URL_FOLDER = ''
	...
	Note: To test Sodo front page after starting Sodo server, set STATIC_ROOT as follows:
		STATIC_ROOT = '{0}{1}'.format('~/sodo-static/static/', STATIC_URL_FOLDER)
...
cd sodo
source bin/activate
git clone https://github.com/ngageoint/sodo.git
cd sodo
sudo -u postgres psql << EOF
create role sodo login password 'sodo';
create database sodo with owner sodo;
\c sodo
...
# Modify sodo/requirements.txt so that the line 'six==1.4.1' reads 'six>=1.6.0'
paver install_dependencies
...
cat << EOF > sodo/local_settings.py
