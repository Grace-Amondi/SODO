...
leaflet_layer_control.addRotationHelper = function($accordion) {
  ...
  var rhdom = $(rhHTML);
  rhdom.appendTo(rh);
  $("#spinnerResetButton").click(function(evt) {
    leaflet_layer_control.rotateMap(0);
    $("#roseSpinnerInput").val(0);
  });
...
leaflet_layer_control.addGeoOverview = function($accordion) {
  ...
  setTimeout(function() {
    minimap.fitBounds(big.getBounds());
  }, 500);
        ...
        .editable(editableUrl, {
          ...
          callback: function(newText, settings) {
            if (feature.layer && feature.layer._icon) {
              ...
              $icon.css({
                width: mapTextSize[0] + 'px',
                height: mapTextSize[1] + 'px'
              });
            }
          }
        });
...
leaflet_layer_control.featureSchemaSelect = function(feature, index) {
  ...
  if (styles && styles.schema && _.isArray(styles.schema)) {
    var schemaInfo = _.find(styles.schema, function(setting) {
      return setting.property == index;
    });
...
leaflet_layer_control.parsers.infoFromLayer = function(obj) {
  ...
  html += leaflet_layer_control.parsers.textIfExists({
    name: obj.name,
    title: "Layer",
    header: true,
    linkit: obj._url,
    linkSuffix: capabilitiesLink
  });
  ...
  if (obj._layers) {
    ...
    if (number_by_analyst) {
      html += leaflet_layer_control.parsers.textIfExists({
        name: number_by_analyst,
        title: "Features you entered"
      });
    }
  ...
  if (obj.options) {
    html += leaflet_layer_control.parsers.textIfExists({
      name: obj.options.attribution
    });
    html += leaflet_layer_control.parsers.textIfExists({
      name: obj.options.layers,
      title: "Layers"
    });
  }
...
leaflet_layer_control.parsers.infoFromInfoObj = function(obj) {
  ...
  html += leaflet_layer_control.parsers.textIfExists({
    ...
    header: true,
    linkit: obj.url,
    linkSuffix: "?request=GetCapabilities"
  });
  html += leaflet_layer_control.parsers.textIfExists({
    name: obj.type,
    title: "Type"
...
leaflet_layer_control.parsers.dynamic_param_parsers.__group_box = function(layer, param, input) {
  "use strict";
  var wrapper = document.createElement("div");
  wrapper.className = "sodo-param-group clearfix";

  var label = document.createElement("label");
  label.textContent = param.name;
  label.className = "sodo-param-group-label";
...
leaflet_layer_control.layerDataList = function(options) {
  ...
  _.each(layerGroups, function(layerGroup, groupNum) {
      ...
      if (!layer.skipThis) {
        ...
        if (showEvenIfNotInUS || layerPreviouslyChosen) {
          if (layer.getLayers && layer.getLayers() && layer.getLayers()[0]) {
            ...
            layer_obj.selected = true;
          } else if (layer.options && layer.options.is_sodo_feature) {
            layer_obj.selected = true;
          }
    ...
    if (nodes && nodes[0] && nodes[0].data && nodes[0].data.options && _.isNumber(nodes[0].data.options.order)) {
      treeData[groupNum].children = _.sortBy(nodes, function(obj) {
        return +obj.data.options.order
      });
    } else {
      treeData[groupNum].children = _.sortBy(nodes, function(obj) {
        return (obj.data && obj.data.name) ? obj.data.name : obj.key;
      });
    }
...
leaflet_layer_control.setLayerOpacity = function(layer, amount, doNotMoveToTop) {
  ...
  if (layer.setStyle) {
    ...
    try {
      ...
      var reset = false;
      try {
        var main_layer = _.find(_.flatten(aoi_feature_edit.layers), function(l) {
...
leaflet_layer_control.addLayerControl = function(map, options, $accordion) {
  ...
  var $tree = $("<div>")
    .attr({
      name: 'layers_tree_control',
      id: 'layers_tree_control'
    });
  ...
  leaflet_layer_control.importNode = $("#layers_tree_control").fancytree("getRootNode").addChildren({
    title: "Imports",
    key: "imports",
    folder: true,
    selected: true
  ...
  if (hidden) {
    var df = _.findWhere(leaflet_layer_control.lastSelectedNodes, {
      title: "Data Feeds"
    });
    if (df) {
...
leaflet_layer_control.highlightFeature = function(e) {
  ...
  if (layer._icon) {
    ...
    layer.setIcon(L.icon({
      iconUrl: "/static/leaflet/images/red-marker-icon.png"
    }));
  }
};
