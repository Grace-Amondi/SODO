...
//            featureLayer.addData(featureCollection);
            ...
            featureLayer.options.is_sodo_feature = true;
...
//            aoi_feature_edit.featureLayers[tnum].addData(featureCollection);
   ...
   aoi_extents.on('click', function (e) {      
    ...
    if (aoi_feature_edit.showPanel(footprints.plugin_title)) {
        footprints.init({
            url_template: '/sodo/api/test/image_footprints/?bbox={{bounds}}',
            layerList: [1],
            title: 'Images',
            markerColorMax: '#ffdf00',
            markerColorMin: '#332200',
            showRejectOption: true,
            featureSelectUrl: '/sodo/api/workcell-image/{{id}}'
        });
    }

    if (aoi_feature_edit.showPanel(imageviewer.plugin_title)) {
        imageviewer.init({
            finishImageUrl: '/sodo/api/workcell-image/{{id}}/examined'
        });
    }
        ...
        success: function (response) {
            ...
            BootstrapDialog.show({
                ...
                buttons: [{
                    ...
                    action: function(dialog) {
                        sodo.redirect(aoi_feature_edit.job_absolute_url);
                    }
                }, {
                    label: 'Work on another Work Cell',
                    action: function(dialog) {
                        sodo.redirect(aoi_feature_edit.next_aoi_url);
                    }
                }]
            });
        },
