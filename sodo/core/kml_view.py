...
from sodo.maps.models import FeatureType
...
class JobKML(ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        doc_name = 'Sodo C:'+str(aoi_complete)+', W:'+str(aoi_work)+', Tot:'+str(aoi_count)+' ['+str(aoi_tot_pct)+'%]'
        ...
        output += '    <Style id="sodo_unassigned">\n'
        output += '      <LineStyle>\n'
        ...
        for feature in feature_types:
            output += '    <Style id="sodo_'+str(feature.id)+'">\n'
            out_color = '7f0066ff'
        ...
        for loc in locations:
            ...
            output += '      <styleUrl>#sodo_'+str(loc.template.id)+'</styleUrl>\n'
            output += '      '+str(kml)+'\n'
            output += '    </Placemark>\n'
        ...
        for aoi in aois:
            ...
            output += '      <styleUrl>#sodo_'+style+'</styleUrl>\n'
            output += '      '+kml+'\n'
            output += '    </Placemark>\n'
...
class JobKMLNetworkLink(ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        url = request.build_absolute_uri('/sodo/api/job/'+id+'.kml' + cookie_url_trailer)
        ...
        output += '<kml xmlns="http://www.opengis.net/kml/2.2">\n'
        output += '  <Folder>\n'
        output += '    <name>Sodo Worked Cells</name>\n'
        output += '    <visibility>1</visibility>\n'
        output += '    <open>1</open>\n'
        output += '    <description>Work progress from Sodo</description>\n'
        output += '    <NetworkLink id="Sodo-'+id+'">\n'
        output += '      <name>'+doc_name+'</name>\n'
        output += '      <visibility>1</visibility>\n'
        output += '      <open>1</open>\n'
        output += '      <description>'+description+'</description>\n'
